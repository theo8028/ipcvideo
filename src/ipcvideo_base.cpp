...
bool writeShareVideo(int ch, char *pSrcImg,int bufSize,int imgWidth,int imgHeight,int imgDepth,int pixelFormat,int *pFrameID)
{
  ...
  {
    g_pSMemVideoHeader[ch]->frameID++;
    if( pFrameID ){
      *pFrameID = g_pSMemVideoHeader[ch]->frameID;
    }
    ...
    g_pSMemVideoTail[ch]->frameID = g_pSMemVideoHeader[ch]->frameID;
    ReleaseMutex(g_lockSMem[ch]);      
    SetEvent(g_eventSMem[ch]);
    slog_info("writeShareVideo :  ch(%d), frameID(%d)\n",ch, g_pSMemVideoHeader[ch]->frameID);
    return true;
  }
  return false;
}
               ...
               int *pPixelFormat,int *pFrameCount,double *pLatencySec )
{
   ...
   if(g_FrameID[ch] == g_pSMemVideoHeader[ch]->frameID)
   {
   ...
   if (dwRet == WAIT_OBJECT_0 || dwRet == WAIT_ABANDONED)
   {
      ...
      *pPixelFormat = g_pSMemVideoHeader[ch]->pixelFormat;
      g_FrameID[ch] = g_pSMemVideoHeader[ch]->frameID;   
      if (pFrameCount) {
         *pFrameCount = g_FrameID[ch];      
      }
      if (pLatencySec)
      {
