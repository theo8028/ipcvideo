...
static PyObject*  read(PyObject* self, PyObject* args)
{
  ...
  if( bufSize > 0 )
  {
    ...
    if( readShareVideo(readChannel,pDstImg,bufSize,&imgWidth,&imgHeight,&imgDepth,&pixelFormat,&frameID,&latencySec )>0 ){
      slog_info("image rx : frameID(%d), latency(%.3fms) \n", frameID, latencySec*1000);
      return array;
    }
    Py_DECREF(array);
  }
  ...
  if( oi != NULL )
  {
    ...
    else if( PyArray_Check(oi) )
    {
      ...
      if (ndims == 3||ndims == 1)
      {
          ...
          writeShareVideo(writeChannel,pData,bufSize,imgWidth, imgHeight,imgDepth,pixelFmt, &frameID);
          ...
          slog_trace("write : ch(%d), frameID(%d)\n",writeChannel,frameID);
          Py_RETURN_TRUE;
      }
      else
      {
